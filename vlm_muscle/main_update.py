...
from utils.vlm_helper import run_remote, deepseek_query, gemini_api
...
def run(args):
    ...
    try:
        ...
        comparison   = file_to_string(os.path.abspath(os.path.join(os.path.dirname(__file__), "./prompts/video_comparison.txt"))).format(task=args.task_description)
            ...
            for attempt in range(args.nattempts):
                try:
                    ...
                    if stage != 0:
                        ...
                        video_analysis = vlm_api(best_video_path, video_prompt)
                    ...
                    if stage == 0: # do a comparison of the previous best video and the current video
                        best_video_path = os.path.join(output_dir_stage, "output_125_video_final.mp4")
                    else: 
                        print("Comparing current video with best video so far")
                        dir_best = os.path.dirname(best_video_path)
                        video_path = os.path.join(output_dir_stage, "output_125_video_final.mp4")
                        vid_comparison = vlm_api([video_path, best_video_path], comparison)
                        logger.info(f"Video Comparison Result: {vid_comparison}")
                        if "first" in vid_comparison.lower():
                            best_video_path = video_path

                        os.makedirs(os.path.join(output_dir_stage, "best"), exist_ok=True)
                        dst_best_dir = os.path.join(output_dir_stage, "best")
                        try:
                            for name in os.listdir(dir_best):
                                src = os.path.join(dir_best, name)
                                dest = os.path.join(dst_best_dir, name)
                                # Avoid copying the destination directory into itself
                                if os.path.abspath(src) == os.path.abspath(dst_best_dir):
                                    continue
                                # Copy only files (and symlinks), skip directories entirely
                                if os.path.isfile(src) or os.path.islink(src):
                                    shutil.copy2(src, dest)
                                else:
                                    logger.info(f"Skipping directory {src}")
                            logger.info(f"Copied files from {dir_best} to {dst_best_dir}")
                        except Exception as e:
                            logger.error(f"Failed to copy files from {dir_best} to {dst_best_dir}: {e}")
                    break
                except Exception as e:
