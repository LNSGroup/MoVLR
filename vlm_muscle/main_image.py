...
def run(args):
    ...
    log_dir      = os.path.join(curr_path, "logs", "image", args.body.split(" ")[0] , args.task_name)
            ...
            for attempt in range(args.nattempts):
                try:
                    ...
                    if stage != 0:
                        ...
                        video_prompt = file_to_string(os.path.abspath(os.path.join(os.path.dirname(__file__), "./prompts/image_prompt.txt"))).format(reward_terms=prev_rewards, task=args.task_description, body=f"{args.body.split(" ")[0]} {species}", focus=focus)
                        
                        # save 4 image snapshots from the video
                        get_image(video_path, 4)
                        image_paths = [os.path.join(os.path.dirname(video_path), f"image{i+1}.png") for i in range(4)]
                        # image_paths = [f"{os.path.dirname(video_path)}/frame_{i}.png" for i in range(4)]
