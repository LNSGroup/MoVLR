...
std::string Rough::XmlPath() const {
  return GetModelPath("musculoskeletal/rough/task.xml");
}
std::string Rough::Name() const { return "MS Rough"; }
...
// -------------------------------------------------------------
void Rough::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  // ----- upright ----- //
  double up_dir[3] = {0*(parameters_[0]), 0, 1};
  ...
  com_velocity_upper[0] -= parameters_[1];
...
void Rough::TransitionLocked(mjModel* model, mjData* data) {
  if (data->time == 0) {
    mju_copy(data->qpos, model->key_qpos, model->nq);
    data->qpos[1] = 0.05; // reset pelvis height
