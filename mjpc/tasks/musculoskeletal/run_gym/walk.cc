...
std::string RunGym::XmlPath() const {
  return GetModelPath("musculoskeletal/run_gym/task.xml");
}
std::string RunGym::Name() const { return "MS RunGym"; }
...
// -------------------------------------------------------------
void RunGym::ResidualFn::Residual(const mjModel* model, const mjData* data,
...
void RunGym::TransitionLocked(mjModel* model, mjData* data) {
  if (data->time == 0) {
