...
std::tuple<int, int, double, double> ComputeInterpolationValues(double index,
  ...
  int index_1 = std::min(index_0 + 20, max_index);
...
constexpr double kFps = 500;

constexpr int kMotionLengths[] = {
    9000,  // Jump - CMU-CMU-02-02_04
...
std::string Tracking5::XmlPath() const {
  return GetModelPath("musculoskeletal/tracking5/task.xml");
}
std::string Tracking5::Name() const { return "MS Track5"; }
...
// ----------------------------------------------------------------
void Tracking5::ResidualFn::Residual(const mjModel *model, const mjData *data,
  ...
  // // ----- joint pos ----- //
  // mjtNum *qpos_0 = new mjtNum[model->nq];
  // mjtNum *qpos_1 = new mjtNum[model->nq];
  // mju_scl(qpos_0, model->key_qpos + model->nq * key_index_0, weight_0, model->nq);
  // mju_scl(qpos_1, model->key_qpos + model->nq * key_index_1, weight_1, model->nq);

  // // mju_copy(d->qpos, qpos_0, model->nq);
  // mju_addTo(qpos_0, qpos_1, model->nq);
  // // std::cout<<mju_norm3(qpos_0+3)<<" "<<mju_norm3(data->qpos+3)<<std::endl;
  // // mju_subFrom(qpos_0, data->qpos, model->nq);
  // // mju_copy(residual, qpos_0+3, model->nq-3);

  // std::vector<int> includeList = {3, 4, 5, 6, 7, 8, 12, 15, 16, 17, 21, 22, 23, 27, 30, 31, 32, 
  // 36, 37, 38, 39, 40, 41, 42, 43, 44, 
  // 55, 56, 58, 59, 60, 61, 62, 75, 76, 78, 79, 80, 81, 82}; 

  // for (int i = 0; i < includeList.size(); ++i) {
  //   residual[counter++] = qpos_0[includeList[i]] - data->qpos[includeList[i]];
  // }

  // std::cout<<mju_norm3(residual)<<std::endl;
  ...
  auto get_body_mpos = [&](const std::string &body_name, double result[3]) {
    ...
    // std::cout<<model->nmocap<<" "<<key_index_0<<" "<<weight_0<<" "<<key_index_1<<" "<<weight_1<<" "<<body_mocapid<<std::endl;
    // current frame
    mju_scl3(
        result,
  ...
  //   double body_mpos[3];
  //    get_body_mpos(body_names[i], body_mpos);
  //   if (mju_abs(body_mpos[0]) < min_x) {
  //     min_x = mju_abs(body_mpos[0]);
  //     min_x_idx = i;
  //   }
  //   if (mju_abs(body_mpos[1]) < min_y) {
  //     min_y = mju_abs(body_mpos[1]);
  //     min_y_idx = i;
  //   }
  // }

  // // std::cout<<body_names[min_x_idx]<<" "<<body_names[min_y_idx]<<std::endl;

  // double min_x_pos[3];
  // double min_y_pos[3];
  // get_body_mpos(body_names[min_x_idx], min_x_pos);
  // get_body_mpos(body_names[min_y_idx], min_y_pos);

  for (const auto &body_name : body_names) {
    double body_mpos[3];
    double body_sensor_pos[3];
    get_body_mpos(body_name, body_mpos);
    // body_mpos[0] -= min_x_pos[0];
    // body_mpos[1] -= min_y_pos[1];
  ...
  // std::cout<<"res "<<data->time<<" "<<counter<<" "<<residual[counter]<<" "<<residual[counter+1]<<" "<<residual[counter+2]<<std::endl;
  counter += 3;
  ...
  for (const auto &body_name : body_names) {
    ...
    // double body_pos_diff[3];
    // mju_sub3(body_pos_diff, body_mpos, body_sensor_pos);
    // body_pos_diff[0] *= 10;
    // body_pos_diff[1] *= 10;
    // mju_copy(&residual[counter], body_pos_diff, 3);
    mju_sub3(&residual[counter], body_mpos, body_sensor_pos);
    // std::cout<<"res "<<data->time<<" "<<counter<<" "<<body_name<<std::endl;
    // residual[counter+2] *= 2;
    counter += 3;
  }
  ...
  for (const auto &body_name : body_names) {
    ...
    mju_subFrom3(
        &residual[counter],
        model->key_mpos + model->nmocap * 3 * key_index_0 + 3 * body_mocapid);
    mju_scl3(&residual[counter], &residual[counter], kFps/20);
...
// ----------------------------------------------------------------------------
void Tracking5::TransitionLocked(mjModel *model, mjData *d) {
  // get motion start index
  int start = MotionStartIndex(mode);
  // get motion trajectory length
  int length = MotionLength(mode);
  // std::cout<<d->time<<" "<<start<<" "<<length<<std::endl;
  // check for motion switch
  if (residual_.current_mode_ != mode || d->time == 0) {
  ...
  // mjtNum *qpos_0 = mj_stackAllocNum(d, model->nq);
  // mjtNum *qpos_1 = mj_stackAllocNum(d, model->nq);
  // mju_scl(qpos_0, model->key_qpos + model->nq * key_index_0, weight_0, model->nq);
  // mju_scl(qpos_1, model->key_qpos + model->nq * key_index_1, weight_1, model->nq);

  // mju_copy(d->qpos, qpos_0, model->nq);
  // mju_addTo(d->qpos, qpos_1, model->nq);
  //   // mju_copy(d->qvel, model->key_qvel + model->nv * start, model->nv);
