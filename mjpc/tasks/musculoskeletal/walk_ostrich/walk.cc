...
std::string WalkOstrich::XmlPath() const {
  return GetModelPath("musculoskeletal/walk_ostrich/task.xml");
}
std::string WalkOstrich::Name() const { return "MS WalkOstrich"; }
...
// -------------------------------------------------------------
void WalkOstrich::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  // std::cout<<"feet cross "<<mju_dot(hip_dir, feet_dir, 2)<<" "<<mju_dot(hip_dir, toe_dir, 2)<<std::endl;
  ...
  // // ----- Joint Pos ----- //
  residual[counter++] = mju_norm(data->qpos+20, model->nq-20);
...
void WalkOstrich::TransitionLocked(mjModel* model, mjData* data) {
