...
std::string StandGym::XmlPath() const {
  return GetModelPath("musculoskeletal/stand_gym/task.xml");
}
std::string StandGym::Name() const { return "MS StandGym"; }
...
// -------------------------------------------------------------
void StandGym::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  // ----- Joint Velocity ----- //
  residual[counter++] = mju_norm(data->qvel, 85);

  // ----- Joint Position ----- //
  double* joint_position = data->qpos;
  double joint_pos = 0;
  for (int i = 0; i < 85; i++) {
    joint_pos += joint_position[i] * joint_position[i];
  }
  residual[counter++] = mju_sqrt(joint_pos);

  // ----- Joint Acc ----- //
  double* joint_acc = data->qacc;
  double jnt_acc = 0;
  for (int i = 0; i < 85; i++) {
    jnt_acc += joint_acc[i] * joint_acc[i];
  }
  residual[counter++] = mju_sqrt(jnt_acc);
...
void StandGym::TransitionLocked(mjModel* model, mjData* data) {
  if (data->time == 0) {
