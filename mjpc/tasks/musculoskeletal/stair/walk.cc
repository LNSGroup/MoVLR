...
std::string Stair::XmlPath() const {
  return GetModelPath("musculoskeletal/stair/task.xml");
}
std::string Stair::Name() const { return "MS Stair"; }
void Stair::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  upright += 0 * (1- mju_dot(torso_up, up_dir, 3));
  ...
  upright += 0. * (1- mju_dot(toe_left_up, up_dir, 3));
  upright += 0. * (1- mju_dot(toe_right_up, up_dir, 3));
  ...
  forward += 0. * (1- mju_dot(toe_left_forward, forward_dir, 3));
  forward += 0. * (1- mju_dot(toe_right_forward, forward_dir, 3));
...
void Stair::TransitionLocked(mjModel* model, mjData* data) {
  if (data->time == 0) {
