...
#include "mjpc/planners/hierarchical_cross_entropy/planner.h"
...
void HierarchicalCrossEntropyPlanner::Initialize(mjModel* model, const Task& task) {
  ...
  // interpolation_ = GetNumberOrDefault(SplineInterpolation::kLinearSpline, model,
...
// allocate memory
void HierarchicalCrossEntropyPlanner::Allocate() {
  // initial state
...
void HierarchicalCrossEntropyPlanner::Reset(int horizon,
  ...
  std::cout<<"init var mean "<<std::accumulate(variance.begin(), variance.end(), 0.0)/variance.size()<<std::endl; 
  // trajectory samples
  for (int i = 0; i < kMaxTrajectory; i++) {
...
// set state
void HierarchicalCrossEntropyPlanner::SetState(const State& state) {
  state.CopyTo(this->state.data(), this->mocap.data(), this->userdata.data(),
               &this->time);
}

// optimize nominal policy using random sampling
void HierarchicalCrossEntropyPlanner::OptimizePolicy(int horizon, ThreadPool& pool) {
  ...
  for (int i = 0; i < n_elite; i++) {
    ...
    for (int t = 0; t < num_spline_points; t++) {
      ...
      for (int j = 0; j < policy.dim_high_level_action; j++) {
        parameters_scratch[t * policy.dim_high_level_action + j] += n.values()[j];
      }
    }
  ...
  for (int t = 0; t < num_spline_points; t++) {
    ...
    for (int j = 0; j < policy.dim_high_level_action; j++) {
      ...
      for (int i = 0; i < n_elite; i++) {
        ...
        double pi = n.values()[j];
        double diff = pi - p_avg;
        variance[t * policy.dim_high_level_action + j] += diff * diff / (n_elite);
        variance[t * policy.dim_high_level_action + j] = std::max(variance[t * policy.dim_high_level_action + j], std_min_ * std_min_);
      }
    }
    std::cout<<"variance mean "<<std::accumulate(variance.begin(), variance.begin()+policy.dim_high_level_action*num_spline_points, 0.0)/policy.dim_high_level_action*num_spline_points<<" "<<variance.size()<<std::endl;
  }
...
void HierarchicalCrossEntropyPlanner::NominalTrajectory(int horizon) {
  // set policy
...
void HierarchicalCrossEntropyPlanner::NominalTrajectory(int horizon, ThreadPool& pool) {
  NominalTrajectory(horizon);
}

// set action from policy
void HierarchicalCrossEntropyPlanner::ActionFromPolicy(double* action, const double* state,
...
void HierarchicalCrossEntropyPlanner::SyncTargetPos(mjData* data) {
  const std::shared_lock<std::shared_mutex> lock(mtx_);
  double act_winner[85];
  
  // mju_copy(act_winner, data->qpos, 85);
  mju_zero(act_winner, 85);
  ...
  Clamp(act_winner, model->jnt_range, 85);

  

    
  mju_copy(data->qpos+85, act_winner, 85);
  mju_zero(data->qvel+85, 85);
  // std::cout<<"copy pos "<<data->qpos[85+6]<<std::endl;
}

// update policy via resampling
void HierarchicalCrossEntropyPlanner::ResamplePolicy(int horizon) {
  // dimensions
  int num_spline_points = resampled_policy.num_spline_points;
...
void HierarchicalCrossEntropyPlanner::AddNoiseToPolicy(int i, double std_min) {
  // start timer
  ...
  for (int k = 0; k < candidate_policy[i].plan.Size(); k++) {
    ...
    //scale by jnt range
    // for (int j = 0; j < policy.dim_high_level_action; j++) {
    //   double scale = 0.5 * (model->jnt_range[2 * policy.includeList[j] + 1] -
    //                         model->jnt_range[2 * policy.includeList[j]]);
    //   if (scale > 1.57) {
    //     scale = 1.57;
    //   }
    //   noise[shift + k * policy.dim_high_level_action + j] = noise[shift + k * policy.dim_high_level_action + j] * scale;
    // }
    // if (k == 6 || k == 12 || k == 15 || k == 21 || k == 27 || k == 30) {
    //     scale = 5;
    //   }
...
void HierarchicalCrossEntropyPlanner::Rollouts(int num_trajectory, int horizon,
                                   ThreadPool& pool) {
  ...
  for (int i = 0; i < num_trajectory; i++) {
    pool.Schedule([&s = *this, &model = this->model, &task = this->task,
      ...
      while(total_ret >= 1.0e6) {
        ...
        Eigen::VectorXd plan_vec = Eigen::Map<Eigen::VectorXd>(s.candidate_policy[i].plan.NodeAt(0).values().data(), s.candidate_policy[i].dim_high_level_action);
        ...
        // std::cout<<"action "<<i<<" "<< plan_vec.sum() <<" "<<s.trajectory[i].total_return<<std::endl;
...
const Trajectory* HierarchicalCrossEntropyPlanner::BestTrajectory() {
  return &nominal_trajectory;
}

//for act from policy
void HierarchicalCrossEntropyPlanner::SyncPolicyState(HierarchicalPDPolicy policy, const double* src, double time) {
...
void HierarchicalCrossEntropyPlanner::Traces(mjvScene* scn) {
  // sample color
  float color[4];
  color[0] = 1.0;
  color[1] = 1.0;
...
void HierarchicalCrossEntropyPlanner::GUI(mjUI& ui) {
...
void HierarchicalCrossEntropyPlanner::Plots(mjvFigure* fig_planner, mjvFigure* fig_timer,
