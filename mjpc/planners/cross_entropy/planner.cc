...
void CrossEntropyPlanner::Initialize(mjModel* model, const Task& task) {
  ...
  num_trajectory_ = GetNumberOrDefault(20, model, "sampling_trajectories");
  ...
  for (int i = 0; i < n_elite; i++) {
    ...
    for (int t = 0; t < num_spline_points; t++) {
      ...
      for (int j = 0; j < model->nu; j++) {
        ...
        parameters_scratch[t * policy.model->nu + j] += weights(i) * n.values()[j] / weight_sum;
      }
    }
  ...
  for (int t = 0; t < num_spline_points; t++) {
    ...
    for (int j = 0; j < model->nu; j++) {
      ...
      for (int i = 0; i < n_elite; i++) {
        ...
        variance[t * model->nu + j] += weights(i) * norm_diff * norm_diff / weight_sum;
        // variance[t * model->nu + j] = std::max(variance[t * model->nu + j], std_min_ * std_min_);
      }
    }
  }
...
void CrossEntropyPlanner::SyncTargetPos(mjData* data) {
}
