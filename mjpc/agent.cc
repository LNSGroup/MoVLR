...
void Agent::PlotInitialize() {
  ...
  std::cout << "cost" << plots_.cost.linergb[0][0] << " "
            << plots_.cost.linergb[0][1] << " " << plots_.cost.linergb[0][2]
            << std::endl;
...
void Agent::Plots(const mjData* data, int shift) {
  ...
  {
      FILE* cost_log = fopen("agent_cost_log.csv", "a");
      if (cost_log) {
          // Write: time, total cost, term1, term2, ...
          fprintf(cost_log, "%f,%f", data->time, cost_);
          for (int k = 0; k < ActiveTask()->num_term; ++k) {
              fprintf(cost_log, ",%f", terms_[k]);
          }
          fprintf(cost_log, "\n");
          fclose(cost_log);
      }
...
void Agent::PlotShow(mjrRect* rect, mjrContext* con) {
  ...
  viewport.bottom += rect->height / 3/2;
  mjr_figure(viewport, &plots_.planner, con);
  viewport.bottom += rect->height / 3/2;
  mjr_figure(viewport, &plots_.action, con);
  // viewport.bottom += rect->height / num_sections;
  mjrRect viewport2 = {rect->left + rect->width - rect->width / num_sections,
                      rect->bottom+rect->height / 3/2*3, rect->width / num_sections,
                      rect->height /2};
  mjr_figure(viewport2, &plots_.cost, con);
  // std::cout<< "Plots shown" << std::endl;
}
