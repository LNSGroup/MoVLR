...
void BaseResidualFn::CostTerms(double* terms, const double* residual,
  ...
  for (int k = 0; k < num_term_; k++) {
    ...
    // double my_term = mju_sqrt(residual[f_shift]*residual[f_shift] + 0.1 * 0.1) - 0.1;
    // std::cout<<k<<" "<<residual[f_shift]<<" "<<terms[k]<<" "<<weight_[k] * my_term<<" "<<norm_parameter_[p_shift]<<std::endl;
    // shift residual
