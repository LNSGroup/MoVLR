...
void State::Allocate(const mjModel* model) {
  ...
  state_.resize(model->nq + model->nv + model->na + 2*model->nu + 6*model->nbody);
...
void State::Set(const mjModel* model, const mjData* data) {
  if (model && data) {
    ...
    //perturb
    SetPerturb(model, data->xfrc_applied);
  }
}
...
void State::SetPerturb(const mjModel* model, const double* xfrc_applied) {
  mju_copy(DataAt(state_, model->nq + model->nv + model->na + 2*model->nu), xfrc_applied, 6*model->nbody);
}
