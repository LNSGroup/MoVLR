...
void Trajectory::Rollout(
    std::function<void(double* action, const double* state, double time)>
        policy,
    const Task* task, const mjModel* model, mjData* data, const double* state,
    double time, const double* mocap, const double* userdata, int steps, double perturb_std) {
  // std::cout<<"rollout1"<<std::endl;
  NoisyRollout(policy, task, model, data, state, time, mocap, userdata,
               /*xfrc_std=*/perturb_std, /*xfrc_rate=*/0, steps);
}
...
void Trajectory::NoisyRollout(
  ...
  times[0] = time;
  data->time = time;
  // std::cout<<"qpos "<<std::accumulate(state, state+nq, 0.0)<<std::endl;
  ...
  for (int t = 0; t < horizon - 1; t++) {
    ...
    // plan with perturb
    mju_copy(data->xfrc_applied, state + nq + nv + na + 2*nu, 6*model->nbody);
    // if (t == 0){
    //   std::cout<<"action "<< std::accumulate(actions.begin()+t*nu, actions.begin()+t*nu+nu, 0.0) <<std::endl;
    ...
    // std::cout<<mju_norm(data->xfrc_applied, 6*model->nbody)<<std::endl;
