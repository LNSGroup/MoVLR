...
const int maxgeom = 500000;            // preallocated geom array in mjvScene
...
void UpdateProfiler(mj::Simulate* sim) {
  ...
  for (int k=0; k < nisland; k++) {
    ...
    for (int i=0; i<sim->figcost.linepnt[0]; i++) {
      ...
      // Log cost values for plotting
      {
          FILE* cost_log = fopen("cost_log.csv", "a");
          if (cost_log) {
              std::cout << "logging cost values" << std::endl;
              fprintf(cost_log, "%f,%f,%f\n", stat->improvement, stat->gradient, stat->lineslope);
              fclose(cost_log);
          }
      }
    }
...
void UpdateInfoText(mj::Simulate* sim,
  ...
  if (best_trajectory) {
    ...
    // double* velocity = mjpc::SensorByName(m, d, "pelvis_subtreelinvel");
    // double* pelvis_pos = mjpc::SensorByName(m, d, "pelvis_position");
    // // double acc = d->cacc[6];
    // // std::string pos_sensor_name = "tracking_pos[pelvis]";
    // // double *pelvis_pos = mjpc::SensorByName(m, d, pos_sensor_name.c_str());
...
void UiEvent(mjuiState* state) {
  ...
  if ((state->dragrect==sim->ui0.rectid) ||
    ...
    if (it) {
      std::cout<<"it: "<<it->sectionid<<std::endl;
    }
...
void Simulate::LoadOnRenderThread() {
  ...
  float desired = mju_log(this->desired_percent_real_time*this->m->vis.global.realtime);
